...
Total Population in 2011 (Continuous — ACS) Total Population in 2013 (Continuous — ACS) Percent change in population (Continuous — calculated (2013 tot_pop - 2011 tot_pop)/(2011 tot_pop)* 100) Percent Race in 2011 (Continuous — ACS) Percent Hispanic in 2011 (Continuous - ACS) Median Income in 2011 (Continuous — ACS) Median Property Value (Continuous — ACS) Percent Renter in 2011 (Continuous — ACS) County (Categorical - ACS) Majority Vote Democrat/Republican 2008 Election (Categorical - MIT Election Lab)  
...
```{r , message=FALSE}
bergen_county_t_test <- t.test(full_acs$county == unique(full_acs$county)[1])
bronx_county_t_test <- t.test(full_acs$county == unique(full_acs$county)[2])
essex_county_t_test <- t.test(full_acs$county == unique(full_acs$county)[3])
fairfield_county_t_test <- t.test(full_acs$county == unique(full_acs$county)[4])
hudson_county_t_test <- t.test(full_acs$county == unique(full_acs$county)[5])
kings_county_t_test <- t.test(full_acs$county == unique(full_acs$county)[6])
middlesex_county_t_test <- t.test(full_acs$county == unique(full_acs$county)[7])
monmouth_county_t_test <- t.test(full_acs$county == unique(full_acs$county)[8])
nassau_county_t_test <- t.test(full_acs$county == unique(full_acs$county)[9])
newhaven_county_t_test <- t.test(full_acs$county == unique(full_acs$county)[10])
newyork_county_t_test <- t.test(full_acs$county == unique(full_acs$county)[11])
ocean_county_t_test <- t.test(full_acs$county == unique(full_acs$county)[12])
queens_county_t_test <- t.test(full_acs$county == unique(full_acs$county)[13])
richmond_county_t_test <- t.test(full_acs$county == unique(full_acs$county)[14])
rockland_county_t_test <- t.test(full_acs$county == unique(full_acs$county)[15])
suffolk_county_t_test <- t.test(full_acs$county == unique(full_acs$county)[16])
union_county_t_test <- t.test(full_acs$county == unique(full_acs$county)[17])
westchester_county_t_test <- t.test(full_acs$county == unique(full_acs$county)[18])

cat_summary_county <- tibble(`County` = 
                              c("Bergen County", "Bronx County", "Essex County",
                                "Fairfield County", "Hudson County", "Kings County",
                                "Middlesex County", "Monmouth County", "Nassau County",
                                "New Haven County", "New York County", "Ocean County",
                                "Queens County", "Richmond County", "Rockland County",
                                "Suffolk County", "Union County", "Westchester County"),
                            `Sample proportion` = 
                              c(bergen_county_t_test$estimate * 100, bronx_county_t_test$estimate *100,
                                essex_county_t_test$estimate *100, fairfield_county_t_test$estimate *100,
                                hudson_county_t_test$estimate *100, kings_county_t_test$estimate *100,
                                middlesex_county_t_test$estimate *100, monmouth_county_t_test$estimate *100,
                                nassau_county_t_test$estimate *100, newhaven_county_t_test$estimate *100,
                                newyork_county_t_test$estimate *100, ocean_county_t_test$estimate *100,
                                queens_county_t_test$estimate *100, richmond_county_t_test$estimate *100,
                                rockland_county_t_test$estimate *100, suffolk_county_t_test$estimate *100,
                                union_county_t_test$estimate *100, westchester_county_t_test$estimate *100),
                            `95-percent confidence - low` = 
                              c(bergen_county_t_test$conf.int[1] * 100, bronx_county_t_test$conf.int[1] * 100,
                                essex_county_t_test$conf.int[1] * 100,fairfield_county_t_test$conf.int[1] * 100,
                                hudson_county_t_test$conf.int[1] * 100, kings_county_t_test$conf.int[1] * 100,
                                middlesex_county_t_test$conf.int[1] * 100, monmouth_county_t_test$conf.int[1] * 100,
                                nassau_county_t_test$conf.int[1] * 100, newhaven_county_t_test$conf.int[1] * 100,
                                newyork_county_t_test$conf.int[1] * 100, ocean_county_t_test$conf.int[1] * 100,
                                queens_county_t_test$conf.int[1] * 100, richmond_county_t_test$conf.int[1] * 100,
                                rockland_county_t_test$conf.int[1] * 100, suffolk_county_t_test$conf.int[1] * 100,
                                union_county_t_test$conf.int[1] * 100, westchester_county_t_test$conf.int[1] * 100),
                            `95-percent confidence - high` = 
                              c(bergen_county_t_test$conf.int[2] * 100, bronx_county_t_test$conf.int[2] * 100,
                                essex_county_t_test$conf.int[2] * 100, fairfield_county_t_test$conf.int[2] * 100,
                                hudson_county_t_test$conf.int[2] * 100, kings_county_t_test$conf.int[2] * 100,
                                middlesex_county_t_test$conf.int[2] * 100, monmouth_county_t_test$conf.int[2] * 100,
                                nassau_county_t_test$conf.int[2] * 100, newhaven_county_t_test$conf.int[2] * 100,
                                newyork_county_t_test$conf.int[2] * 100, ocean_county_t_test$conf.int[2] * 100,
                                queens_county_t_test$conf.int[2] * 100, richmond_county_t_test$conf.int[2] * 100,
                                rockland_county_t_test$conf.int[2] * 100, suffolk_county_t_test$conf.int[2] * 100,
                                union_county_t_test$conf.int[2] * 100, westchester_county_t_test$conf.int[2] * 100))

kable(cat_summary_county, digits = 1)
```
```{r , message=FALSE}
...
```{r , message=FALSE}
black_model <- lm(pct_pop_changeE ~ pct_black_pop_2011E, data = full_acs)
white_model <- lm(pct_pop_changeE ~ pct_white_pop_2011E, data = full_acs)
asian_model <- lm(pct_pop_changeE ~ pct_asian_pop_2011E, data = full_acs)
hispanic_model <- lm(pct_pop_changeE ~ pct_hispanic_pop_2011E, data = full_acs)
renter_model <- lm(pct_pop_changeE ~ pct_renter_2011E, data = full_acs)
income_model <- lm(pct_pop_changeE ~ med_income_2011E, data = full_acs)
propvalue_model <- lm(pct_pop_changeE ~ med_propvalue_2011E, data = full_acs)

summary(black_model)
summary(white_model)
summary(asian_model)
summary(hispanic_model)
summary(renter_model)
summary(income_model)
summary(propvalue_model)
...
party_model <- lm(pct_pop_changeE ~ majority_vote, data = full_acs)
